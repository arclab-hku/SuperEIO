...
void Super_EventPoint::top_k_keypoints(std::vector<std::vector<int>> &keypoints, std::vector<float> &scores, int k, int &MIN_DIST, cv::Mat &mask) {
    std::vector<std::vector<int>> keypoints_top_k;
    std::vector<float> scores_top_k;
    std::vector<size_t> indexes = sort_indexes(scores);
    
    int iter_count = std::min(k, static_cast<int>(keypoints.size()));
...
bool Super_EventPoint::process_output(const BufferManager &buffers, Eigen::Matrix<double, 259, Eigen::Dynamic> &features, int &num_keypoints, std::vector<cv::Point2f> &n_pts, int &MIN_DIST, cv::Mat &mask) {
    ...
    top_k_keypoints(keypoints_, scores_vec, num_keypoints, MIN_DIST, mask);
    ...
    for (int i = 1; i < 3; ++i) {
        for (int j = 0; j < keypoints_.size(); ++j) {
            if (i == 1) { 
                features(i, j) = keypoints_[j][i-1]; //* w_scale
            } else if (i == 2) { 
                features(i, j) = keypoints_[j][i-1]; //* h_scale
            }
        }
    }
    for (int m = 3; m < 259; ++m) {
