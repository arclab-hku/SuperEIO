...
std::shared_ptr<PointMatching> super_eventmatch;
...
void process_glue()
{
    ...
    while (true)
    {
        ...
        if(!image_buf.empty() && !point_buf.empty() && !pose_buf.empty()) //对齐操作，point_msg 和 image_msg 都是对齐后最新的单帧数据
        {
        ...
        if (pose_msg != NULL)
        {
            ...
            if(!( image.rows==ROW &&  image.cols==COL)){//solve image size problem
                std::cout << "image row = " << image.rows << " col = " << image.cols << std::endl;
                std::cout << "set row = " << ROW << " col = " << COL << std::endl;
            ...
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                KeyFrame* keyframe = new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, image,
                                   point_3d, point_2d_uv, point_2d_normal, point_id, sequence, super_eventpoint);

                m_process.lock();
                start_flag = 1;
                posegraph.addKeyFrame_glue(keyframe, 1, super_eventmatch);
                m_process.unlock();
                frame_index++;
                last_t = T;
            }
...
int main(int argc, char **argv)
{
    ...
    super_eventmatch = std::shared_ptr<PointMatching>(new PointMatching(configs.super_eventmatch_config));
    std::cout << "build super_eventmatch" << std::endl;
    ...
    if (LOOP_CLOSURE)
    {
        ...
        // DBoW2
        std::string pkg_path = ros::package::getPath("loop_closure");
        ...
        m_camera = camodocal::CameraFactory::instance()->generateCameraFromYamlFile(config_file.c_str());// camera model

        fsSettings["loop_closure_topic"] >> LOOP_TOPIC;
        fsSettings["loop_closure_save_path"] >> POSE_GRAPH_SAVE_PATH;
    ...
    ros::Subscriber sub_imu_forward = n.subscribe("/supereio_ba/imu_propagate", 2000, imu_forward_callback);//pub imu-rate odom
    ros::Subscriber sub_vio = n.subscribe("/supereio_ba/odometry", 2000, vio_callback);
    ros::Subscriber sub_image = n.subscribe(LOOP_TOPIC, 2000, loop_callback);
    ros::Subscriber sub_pose = n.subscribe("/supereio_ba/keyframe_pose", 2000, pose_callback);
    ros::Subscriber sub_extrinsic = n.subscribe("/supereio_ba/extrinsic", 2000, extrinsic_callback);
    ros::Subscriber sub_point = n.subscribe("/supereio_ba/keyframe_point", 2000, point_callback);
    ros::Subscriber sub_relo_relative_pose = n.subscribe("/supereio_ba/relo_relative_pose", 2000, relo_relative_pose_callback);
    ros::Subscriber sub_event_feature=n.subscribe("/event_detector/event_feature_loop", 2000, &eventsfeatureCallback);//callback event-corner feature
    ...
    std::thread measurement_process= std::thread(process_glue);//loop main function
