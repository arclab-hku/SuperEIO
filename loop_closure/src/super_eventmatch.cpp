...
Super_EventMatch::Super_EventMatch(const Super_EventMatchConfig &super_eventmatch_config) : super_eventmatch_config_(super_eventmatch_config), engine_(nullptr) {
    setReportableSeverity(Logger::Severity::kINTERNAL_ERROR);
}

bool Super_EventMatch::build() {
    ...
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[0].c_str(), OptProfileSelector::kMIN, Dims3(1, 1, 2));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[0].c_str(), OptProfileSelector::kOPT, Dims3(1, 512, 2));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[0].c_str(), OptProfileSelector::kMAX,
                           Dims3(1, 1024, 2));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[1].c_str(), OptProfileSelector::kMIN, Dims2(1, 1));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[1].c_str(), OptProfileSelector::kOPT, Dims2(1, 512));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[1].c_str(), OptProfileSelector::kMAX, Dims2(1, 1024));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[2].c_str(), OptProfileSelector::kMIN, Dims3(1, 256, 1));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[2].c_str(), OptProfileSelector::kOPT,
                           Dims3(1, 256, 512));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[2].c_str(), OptProfileSelector::kMAX,
                           Dims3(1, 256, 1024));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[3].c_str(), OptProfileSelector::kMIN, Dims3(1, 1, 2));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[3].c_str(), OptProfileSelector::kOPT, Dims3(1, 512, 2));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[3].c_str(), OptProfileSelector::kMAX,
                           Dims3(1, 1024, 2));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[4].c_str(), OptProfileSelector::kMIN, Dims2(1, 1));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[4].c_str(), OptProfileSelector::kOPT, Dims2(1, 512));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[4].c_str(), OptProfileSelector::kMAX, Dims2(1, 1024));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[5].c_str(), OptProfileSelector::kMIN, Dims3(1, 256, 1));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[5].c_str(), OptProfileSelector::kOPT,
                           Dims3(1, 256, 512));
    profile->setDimensions(super_eventmatch_config_.input_tensor_names[5].c_str(), OptProfileSelector::kMAX,
                           Dims3(1, 256, 1024));
...
bool Super_EventMatch::construct_network(TensorRTUniquePtr<nvinfer1::IBuilder> &builder,
                                  TensorRTUniquePtr<nvinfer1::INetworkDefinition> &network,
                                  TensorRTUniquePtr<nvinfer1::IBuilderConfig> &config,
                                  TensorRTUniquePtr<nvonnxparser::IParser> &parser) const {
    auto parsed = parser->parseFromFile(super_eventmatch_config_.onnx_file.c_str(),
    ...
    enableDLA(builder.get(), config.get(), super_eventmatch_config_.dla_core);
    return true;
}

bool Super_EventMatch::infer(const Eigen::Matrix<double, 259, Eigen::Dynamic> &features0,
    ...
    const int keypoints_0_index = engine_->getBindingIndex(super_eventmatch_config_.input_tensor_names[0].c_str());
    const int scores_0_index = engine_->getBindingIndex(super_eventmatch_config_.input_tensor_names[1].c_str());
    const int descriptors_0_index = engine_->getBindingIndex(super_eventmatch_config_.input_tensor_names[2].c_str());
    const int keypoints_1_index = engine_->getBindingIndex(super_eventmatch_config_.input_tensor_names[3].c_str());
    const int scores_1_index = engine_->getBindingIndex(super_eventmatch_config_.input_tensor_names[4].c_str());
    const int descriptors_1_index = engine_->getBindingIndex(super_eventmatch_config_.input_tensor_names[5].c_str());
    const int output_score_index = engine_->getBindingIndex(super_eventmatch_config_.output_tensor_names[0].c_str());
    ...
    ASSERT(super_eventmatch_config_.input_tensor_names.size() == 6);
...
bool Super_EventMatch::process_input(const BufferManager &buffers,
                              const Eigen::Matrix<double, 259, Eigen::Dynamic> &features0,
                              const Eigen::Matrix<double, 259, Eigen::Dynamic> &features1) {
    auto *keypoints_0_buffer = static_cast<float *>(buffers.getHostBuffer(super_eventmatch_config_.input_tensor_names[0]));
    auto *scores_0_buffer = static_cast<float *>(buffers.getHostBuffer(super_eventmatch_config_.input_tensor_names[1]));
    auto *descriptors_0_buffer = static_cast<float *>(buffers.getHostBuffer(super_eventmatch_config_.input_tensor_names[2]));
    auto *keypoints_1_buffer = static_cast<float *>(buffers.getHostBuffer(super_eventmatch_config_.input_tensor_names[3]));
    auto *scores_1_buffer = static_cast<float *>(buffers.getHostBuffer(super_eventmatch_config_.input_tensor_names[4]));
    auto *descriptors_1_buffer = static_cast<float *>(buffers.getHostBuffer(super_eventmatch_config_.input_tensor_names[5]));
...
bool Super_EventMatch::process_output(const BufferManager &buffers,
                               Eigen::VectorXi &indices0,
                               Eigen::VectorXi &indices1,
                               Eigen::VectorXd &mscores0,
                               Eigen::VectorXd &mscores1) {
    ...
    mscores0_.clear();
    mscores1_.clear();
    auto *output_score = static_cast<float *>(buffers.getHostBuffer(super_eventmatch_config_.output_tensor_names[0]));
...
void Super_EventMatch::save_engine() {
    if (super_eventmatch_config_.engine_file.empty()) return;
    if (engine_ != nullptr) {
        nvinfer1::IHostMemory *data = engine_->serialize();
        std::ofstream file(super_eventmatch_config_.engine_file, std::ios::binary);;
        if (!file) return;
        file.write(reinterpret_cast<const char *>(data->data()), data->size());
    }
}

bool Super_EventMatch::deserialize_engine() {
    std::ifstream file(super_eventmatch_config_.engine_file, std::ios::binary);
    if (file.is_open()) {
